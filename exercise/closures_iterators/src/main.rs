...
fn main() {
    ...
    // (0, 1), it should return (1, 1). Run the code and make sure it works.
     ...
     pairs
         .into_iter()
         .map( |(x,y)| ({x+1},y) )//destructures tuple
         //.map(|t| (t.0+1, t.1)) //keeps the tuple intact if you need it later.
    ...
    // Hint 2: `x` will be a mutable reference, so remember to dereference it to use it
     ...
     for x in &mut numbers { // numbers.iter_mut() is the longer format
...
println!("This is the challenge exercises. The output should be the same as above.");
    // let pairs = vec![(0, 1), (2, 3), (4, 5)];
    // for z in pairs {
    //     //z.map( |(x,y)| ({x+1},y) ); //destructures tuple
    //     z.map(|t| (t.0+1, t.1)); //keeps the tuple intact if you need it later.
    //     println!("{:?}", z)
    // }; //end for loop

    // let mut numbers = vec![1, 2, 3, 4];
    // numbers
    //     .iter_mut() // numbers.iter_mut() is the longer format
    //     .map(|x| *x * 3) // multiply the value by 3 via the mutable reference x
    //     .collect::<Vec<_>>();
    // println!("{:?}", numbers);// should print [3, 6, 9, 12]
